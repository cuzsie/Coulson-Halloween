...
public class GameControllerScript : MonoBehaviour
{
	...
	private void Update()
	{
		if (this.notebooks >= 7)
		{
			objectiveText.text = "Current Objective: Get to the Center!";
		}
		else if (Started)
		{
			objectiveText.text = "Current Objective: Get the Spooky Pizzas!\n" + notebooks + "/7";
		}
		else
		{
			objectiveText.text = "";
	...
	{
		if (this.mode == "story" || this.mode == "afterdark")
		{
			this.notebookCount.text = this.notebooks.ToString() + "/7 SPOOPY Pizzas";
		}
		else
		{
			this.notebookCount.text = this.notebooks.ToString() + " SPOOPY Pizzas";
		}
		if (this.notebooks == 7 & this.mode == "story" || this.notebooks == 7 & this.mode == "afterdark")
		{
			this.ActivateFinaleMode();
			centerNotif.SetTrigger("Jump");
		}
	}
	...
	private void ActivateFinaleMode()
	{
		...
		finaleTeleporter.SetActive(true);
	}
	...
	public void DeactivateLearningGame(GameObject subject)
	{
		...
		{
			// put some sory of notifying thing here (maybe a notification from the top of the screen?)
		}
	}
	...
	private void IncreaseItemSelection()
	{
		...
		this.itemSelect.transform.position = itemSlot[this.itemSelected].transform.position; //Moves the item selector background(the red rectangle)
		this.UpdateItemName();
	}
	...
	public GameObject finaleTeleporter;
